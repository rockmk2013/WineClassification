...
ggplot(than6,aes(as.factor(GoodQuality),as.numeric(ratio),fill=response))+geom_bar(stat = "identity")+geom_text(label=round(as.numeric(than6$ratio),2),vjust=-0.5,size=8)+xlab("酒類品質")+ylab("Ratio")+ggtitle("Quality>=6 or not")+theme_bw()+theme(plot.title = element_text(hjust = 0.5,size=16),axis.text.x = element_text(size=14))+scale_x_discrete(labels=c("Quality<6", "Quality>=6"))
...
```{r}
sapply(features[1:6], function(x){
  par(mfrow=c(2,3))
  formula = formula(paste0('wine~','',x,''), env = new.env())
  r = roc(formula, wine_)
  plot(r, title = x)
  par(mfrow=c(1,1))
})
