...
ggplot(ratio_table,aes(Var1,as.numeric(Freq)))+geom_bar(stat="identity",fill="#00CACA")+geom_text(aes(label=round(as.numeric(Freq),3)))+ylab("Ratio")+xlab("Wine Quality Level")+ggtitle("Level Distribution")+theme_bw()+theme(plot.title = element_text(hjust=0.5,size=14))
```
...
#console
Accuracy(test_set$quality_level, test_original)
Recall(test_set$quality_level,test_original, positive = "high")
Precision(test_set$quality_level,test_original, positive = "high")
```
